...
Menu::Menu() : currentState(MenuState::MAIN_MENU), selectedIndex(0), selectedDifficulty(0), selectedDifficultyElement(2) {
...
void Menu::initialize() {
  ...
  std::cout << "[DEBUG] Confirm text: '" << confirmButton.getString().toAnsiString() << "'" << std::endl;
  std::cout << "[DEBUG] Back text: '" << backButton.getString().toAnsiString() << "'" << std::endl;
...
void Menu::setState(MenuState state) {
  std::cout << "Menu - zmiana stanu na: " << static_cast<int>(state) << std::endl;
...
void Menu::updateDifficultyDisplay() {
  ...
  confirmButton.setOrigin(confirmBounds.width / 2.0f, confirmBounds.height / 2.0f);
  confirmButton.setPosition(WINDOW_WIDTH / 2.0f + 80.0f, 500.0f);
  
  std::cout << "Zaktualizowano wyświetlanie trudności: Level " << selectedDifficulty << std::endl;
}
...
void Menu::moveUp() {
  ...
  std::cout << "Menu - wybrano opcję: " << selectedIndex << " (" << menuItems[selectedIndex] << ")" << std::endl;
}
...
MenuAction Menu::handleSelection() {
  ...
  if (currentState == MenuState::DIFFICULTY_SELECTION) {
    if (selectedDifficultyElement == 2) {  // Confirm
      std::cout << "Menu - potwierdzono poziom trudności: " << selectedDifficulty << std::endl;
      return MenuAction::CONFIRM_DIFFICULTY;
    } else if (selectedDifficultyElement == 1) {  // Back
      std::cout << "Menu - powrót do menu głównego z wyboru trudności" << std::endl;
    ...
    // Dla selektora poziomu Enter przenosi na Confirm
      ...
      std::cout << "[DEBUG] Enter na selektorze - przejście do Confirm" << std::endl;
  ...
  // Dla innych ekranów
  if (!menuItems.empty()) {
    std::cout << "Menu - potwierdzono wybór: " << menuItems[selectedIndex] << std::endl;
  }
  ...
  switch (currentState) {
    case MenuState::MAIN_MENU:
      switch (selectedIndex) {
        ...
        case 1: return MenuAction::HIGH_SCORES;
    ...
    case MenuState::GAME_OVER:
      switch (selectedIndex) {
        ...
        case 1: return MenuAction::MAIN_MENU;
...
void Menu::handleEvent(const sf::Event& event) {
  ...
  if (currentState == MenuState::DIFFICULTY_SELECTION) {
    ...
    // Obsługa myszy - najechanie kursorem (aktualizacja selectedDifficultyElement)
...
int Menu::checkDifficultyClick(float mouseX, float mouseY) const {
  ...
  std::cout << "[DEBUG] Sprawdzanie kliknięcia w pozycji: (" << mouseX << ", " << mouseY << ")" << std::endl;
