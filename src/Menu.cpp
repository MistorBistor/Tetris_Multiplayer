...
// 🔊 MENU SFX (tylko w tym pliku – nie ruszamy Menu.h jeśli nie chcesz)
...
void Menu::initialize() {
  ...
  if (!sbMenuMove.loadFromFile("../resources/audio/Tetris_move_piece.ogg")) {
    std::cout << "[ERROR] Nie udało się załadować menu move sfx!" << std::endl;
  }
  if (!sbMenuSelect.loadFromFile(
  ...
  if (!sbMenuBack.loadFromFile("../resources/audio/Tetris_line_clear.ogg")) {
    std::cout << "[ERROR] Nie udało się załadować menu back sfx!" << std::endl;
  }
...
void Menu::render(sf::RenderWindow &window) const {
  ...
  if (currentState == MenuState::DIFFICULTY_SELECTION) {
    ...
    if (selectedDifficultyElement == 0) {
      ...
      leftCopy.setCharacterSize(50);
      diffCopy.setCharacterSize(40);
      rightCopy.setCharacterSize(50);
      ...
      confirmCopy.setCharacterSize(30);
