...
void Score::addScore (int lines, int currentLevel) {
    ...
    int mult = std::max (1, currentLevel + 1);   // level 0 -> mnożnik 1
    int linePoints = basePoints * mult;

    // Zwiększamy combo
    comboCount++;

    // Punkty za combo (50 * combo * level)
    int comboPoints = 50 * comboCount * mult;
...
void Score::render (sf::RenderWindow& window) const {
    ...
    // PANEL TIME - nad LEVEL
    sf::RectangleShape timePanel (sf::Vector2f (panelWidth, 80));
    timePanel.setPosition (scoreX, 480); // 570 - 90
    ...
    timeTitle.setPosition (scoreX + 20, 490);
    ...
    timeValue.setPosition (scoreX + 20, 520);
...
void Score::setTheme (const ColorTheme& theme) {
    currentTheme = theme;
}
